...
namespace Kmplete
{
    ...
    void PostProcessingFrameListener::_InitializePipeline(Graphics::VulkanLogicalDevice& vulkanDevice, const Graphics::VulkanContext& vulkanContext)
    {
        ...
        const auto shaderPostStages = shaderManager.GetShaderStageCreateInfos({
            { VertexShaderResolveModule_SID, VK_ShaderStage_Vertex, "main" },
            { FragmentShaderResolveModule_SID, VK_ShaderStage_Fragment, "main" }
        });
    ...
    void PostProcessingFrameListener::_RenderImGui()
    {
        ...
        else
        {
            colorAttachmentInfo = vulkanTextureAttachmentManager.GetRenderingAttachmentInfo(
                Graphics::VKPresets::RenderingAttachmentInfo_Color_LoadStore,
                ColorAttachmentResolve, 0ULL, VK_Resolve_None, VK_ImageLayout_AttachmentOptimal, "swapchain image for non-MSAA"_true, "use swapchain SRGB"_false
            );
        }
