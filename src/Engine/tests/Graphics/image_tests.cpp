...
TEST_CASE("Image from invalid filepath", "[graphics][image]")
{
    ...
    REQUIRE_THROWS(image = CreateUPtr<Image>(Filepath(), ImageChannels::Unknown));
}
//--------------------------------------------------------------------------
...
TEST_CASE("Image from valid pixel buffer invalid parameters", "[graphics][image]")
{
    ...
    REQUIRE_THROWS(image = CreateUPtr<Image>(&iconBuffer[0], iconBufferSize, Math::Size2I(6, 4), ImageChannels::RGBAlpha));

    // size dimension is zero
    REQUIRE_THROWS(image = CreateUPtr<Image>(&iconBuffer[0], iconBufferSize, Math::Size2I(4, 0), ImageChannels::RGBAlpha));

    // buffer size is zero
    REQUIRE_THROWS(image = CreateUPtr<Image>(&iconBuffer[0], 0, Math::Size2I(4, 2), ImageChannels::RGBAlpha));
}
//--------------------------------------------------------------------------
...
TEST_CASE("Image from nullptr file buffer", "[graphics][image]")
{
    ...
    REQUIRE_THROWS(image = CreateUPtr<Image>(nullptr, 32, ImageChannels::RGBAlpha));
}
//--------------------------------------------------------------------------