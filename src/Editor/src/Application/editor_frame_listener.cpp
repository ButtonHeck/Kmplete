...
namespace Kmplete
{
    ...
    {
        const auto& defaultFontAsset = _assetsManager.GetFontManager().GetFontAsset("OpenSans-Regular.ttf"_sid);
        _imguiImpl->AddFont(defaultFontAsset.GetFont().GetBuffer(), scale);

        const auto& iconsFontAsset = _assetsManager.GetFontManager().GetFontAsset("forkawesome-webfont.ttf"_sid);
